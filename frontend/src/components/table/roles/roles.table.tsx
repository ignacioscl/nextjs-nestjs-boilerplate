...
const headers = [
  { name: 'Name', styles: 'min-w-[260px] text-sm text-left' },
  { name: 'Type', styles: 'min-w-[150px] text-sm text-left' },
  { name: 'Period', styles: 'min-w-[250px] text-sm text-left' },
  { name: 'Duration', styles: 'min-w-[130px] text-sm text-left' },
  { name: 'Status', styles: 'min-w-[150px] text-sm text-left' },
  { name: 'Actions', styles: 'min-w-[150px] text-sm text-right' },
]

const Table9 = () => {
    const fetchPosts = async () => {
        const { data } = await axios.get('/api/roles'); // Endpoint de API
        return data;
      };
  return (
    <section className='bg-gray-2 dark:bg-dark py-20 lg:py-[120px]'>
        <Button className='mb-10' onClick={fetchPosts}>Add Role</Button>
      ...
      <tr className='bg-[#F6F8FB] dark:bg-dark-3'>
        {headers.map((header:any, index:number) => (
          <th
            ...
            key={index}
          >
            <p className='text-sm font-medium text-body-color dark:text-dark-7'>
              {header.name}
            </p>
          </th>
        ))}
    ...
    <tbody>
      {data.map((row:any, index:number) => (
        <tr className='border-b border-stroke dark:border-dark-3' key={index}>
          ...
          <td className='py-5 px-4'>
            {row.status ? (
              <span className='inline-flex items-center justify-center h-8 px-5 text-base text-white rounded bg-green'>
                Approved
              </span>
            ) : (
              <span className='inline-flex h-8 items-center justify-center rounded bg-[#F13426] px-5 text-base text-white'>
                Declined
              </span>
            )}
          </td>
...
const Dropdown = ({data}:any) => {
  ...
  return (
    <div className='relative'>
      ...
      >
        <button onClick={() => {console.log(data.title);setDropdownOpen(false)}} className='w-full px-3 py-2 text-sm text-left rounded text-body-color hover:bg-gray-2 dark:text-dark-6 dark:hover:bg-dark-2'>
          Edit {data.title}
        </button>
